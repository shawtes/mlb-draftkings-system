...
# # SEPARATE FLAG FOR GARCH - DON'T OVERWRITE BASIC RISK ENGINE AVAILABILITY
# try:
#     from dfs_garch_volatility_engine import DFSGARCHVolatilityEngine
#     from historical_volatility_engine import historical_engine
#     GARCH_ENGINE_AVAILABLE = True
#     print("✅ Advanced GARCH and Historical Volatility engines loaded successfully!")
# except ImportError as e:
#     GARCH_ENGINE_AVAILABLE = False
#     print(f"⚠️ Advanced volatility engines not available: {e}")
#     print("💡 Basic position sizing (Kelly, bankroll management) still available!")
...
#     # Add MUCH MORE aggressive noise for true diversity
#     diversity_factor = random.uniform(0.1, 0.25)  # 10-25% noise, randomly chosen per candidate
#     noise = np.random.normal(1.0, diversity_factor, len(df))
#     df['Predicted_DK_Points'] = df['Predicted_DK_Points'] * noise
    
#     # Apply additional diversity techniques
#     use_efficiency = random.random() < 0.4  # 40% chance
#     use_contrarian = random.random() < 0.2  # 20% chance to boost lower-owned players
#     use_ceiling = random.random() < 0.3  # 30% chance to prefer high-ceiling players
    
#     # Contrarian approach: occasionally boost lower-projected players
#     if use_contrarian:
#         # Boost bottom 30% of players by salary efficiency
#         bottom_threshold = df['Predicted_DK_Points'].quantile(0.3)
#         boost_mask = df['Predicted_DK_Points'] <= bottom_threshold
#         df.loc[boost_mask, 'Predicted_DK_Points'] *= random.uniform(1.2, 1.5)
    
#     # High-ceiling approach: occasionally add extra variance to high-upside players
#     if use_ceiling:
#         # Add extra variance to players with high ceiling potential
#         top_threshold = df['Predicted_DK_Points'].quantile(0.7)
#         ceiling_mask = df['Predicted_DK_Points'] >= top_threshold
#         ceiling_boost = np.random.normal(1.0, 0.15, ceiling_mask.sum())
...
#     def __init__(self, df_players, salary_cap, position_limits, included_players, stack_settings, min_exposure, max_exposure, min_points, monte_carlo_iterations, num_lineups, team_selections, min_unique=0, bankroll=1000, risk_tolerance='medium', disable_kelly=False, min_salary=None):
#         super().__init__()
...
#             candidate_multiplier = max(3, min(5, self.num_lineups // 20))  # Reduced multiplier
#             candidate_multiplier = max(candidate_multiplier, 3)  # Minimum of 3x
...
#             # ULTRA-AGGRESSIVE LOGIC: Guarantee at least 5-10 lineups minimum
#             if kept_lineups:
#                 most_recent_players = kept_lineups[-1]
#                 unique_players = len(current_players - most_recent_players)
                
#                 # PRIORITY: Always keep first 10 lineups if possible, then apply constraints
#                 if kept_count < 10:
#                     # For first 10 lineups, be extremely lenient
#                     is_unique_enough = (unique_players >= 0) or True  # Almost always accept
#                 elif kept_count < 20:
#                     # For next 10, require at least some difference
#                     is_unique_enough = (unique_players >= max(1, min_unique - 2))
#                 else:
#                     # For the rest, apply normal (but still lenient) logic
#                     if min_unique == 1:
#                         is_unique_enough = (unique_players >= 1)
#                     elif min_unique <= 3:
#                         is_unique_enough = (unique_players >= max(1, min_unique - 1))
#                     else:
#                         is_unique_enough = (unique_players >= min_unique - 1)
#             else:
...
#                 # Only keep the last 3 lineups for comparison to be more lenient
#                 if len(kept_lineups) > 3:
#                     kept_lineups = kept_lineups[-3:]
        
#         kept_ratio = len(filtered_results) / len(results) if results else 0
#         logging.info(f"🎯 ULTRA-AGGRESSIVE MIN UNIQUE: Kept {len(filtered_results)} out of {len(results)} lineups ({kept_ratio:.1%}) (min_unique={min_unique})")
        
#         # SAFETY MECHANISM: Guarantee at least 5 lineups if we have enough input
#         if len(filtered_results) < 5 and len(results) >= 5:
#             logging.warning(f"🚨 EMERGENCY MODE: Only {len(filtered_results)} lineups kept, forcing at least 5")
#             # Take the top 5 lineups regardless of uniqueness constraint
#             sorted_results = sorted(results.items(), key=lambda x: x[1]['total_points'], reverse=True)
#             filtered_results = {}
#             for i, (key, lineup_data) in enumerate(sorted_results[:5]):
...
#     def load_file(self):
#         """Load player data from CSV file with advanced GARCH and volatility analysis"""
...
#                 # 🔥 PERFORM ADVANCED GARCH AND VOLATILITY ANALYSIS 🔥
#                 logging.info("🔄 Performing advanced GARCH and volatility analysis using probability predictions...")
#                 self.df_players = self.perform_advanced_volatility_analysis(self.df_players)
                
#                 self.populate_player_tables()
                
#                 # Enhanced status message with volatility info
#                 vol_cols = [col for col in self.df_players.columns if 'volatility' in col.lower() or 'garch' in col.lower()]
#                 vol_count = len(vol_cols)
                
#                 status_msg = f'✅ Players loaded: {len(self.df_players)} players'
#                 if vol_count > 0:
#                     status_msg += f' with {vol_count} volatility metrics'
                    
#                 self.status_label.setText(status_msg)
#                 logging.info(f"✅ Load complete with advanced volatility analysis")
...
#     def perform_advanced_volatility_analysis(self, df):
#         """
#         🔥 COMPREHENSIVE GARCH AND VOLATILITY ANALYSIS USING HISTORICAL DATA + PROBABILITY PREDICTIONS 🔥
#         This function implements sophisticated financial modeling using real game data and probability analysis
#         """
#         try:
#             # 📊 STEP 1: Load Historical Game Data from merged_output.csv
#             historical_data_loaded = False
#             if GARCH_ENGINE_AVAILABLE:
#                 try:
#                     logging.info("🚀 Loading historical game data from merged_output.csv...")
#                     historical_data_loaded = historical_engine.load_historical_data()
#                     if historical_data_loaded:
#                         logging.info("✅ Historical game data loaded successfully!")
#                         # Add historical volatility metrics to DataFrame
#                         df = historical_engine.calculate_enhanced_player_metrics(df)
#                         logging.info(f"📈 Enhanced {len(df)} players with historical volatility metrics")
#                     else:
#                         logging.warning("⚠️ Could not load historical data - using probability-based analysis only")
#                 except Exception as e:
#                     logging.warning(f"⚠️ Historical volatility engine error: {e}")
#             else:
#                 logging.info("💡 Historical volatility engine not available - using basic position sizing with probability analysis")
            
#             # 💡 STEP 2: Check for probability columns from the CSV for implied volatility
#             prob_columns = [col for col in df.columns if col.startswith('Prob_Over_')]
            
#             if not prob_columns and not historical_data_loaded:
#                 logging.warning("⚠️ No probability columns or historical data found - using basic volatility estimation")
#                 df['Variance'] = (df['Predicted_DK_Points'] * 0.3) ** 2
#                 df['GARCH_Volatility'] = df['Predicted_DK_Points'] * 0.3
#                 df['Historical_Volatility'] = df['Predicted_DK_Points'] * 0.3
#                 df['Volatility_Percentile'] = 0.5
#                 df['Downside_Risk'] = df['Predicted_DK_Points'] * 0.2
#                 df['Upside_Potential'] = df['Predicted_DK_Points'] * 0.4
#                 df['VaR_95'] = df['Predicted_DK_Points'] * 0.7
#                 return df
            
#             if prob_columns:
#                 logging.info(f"💡 Found {len(prob_columns)} probability columns for implied volatility")
#             if historical_data_loaded:
#                 excellent_hist = len(df[df.get('hist_data_quality', '') == 'excellent'])
#                 good_hist = len(df[df.get('hist_data_quality', '') == 'good'])
#                 logging.info(f"📊 Historical data: {excellent_hist} excellent, {good_hist} good quality players")
            
#             logging.info(f"🔬 Starting hybrid volatility analysis (Historical + Implied)...")
#             logging.info(f"🎯 Probability columns: {prob_columns}")
            
#             # Initialize result arrays for vectorized calculations
...
#                 try:
#                     player_name = player['Name']
#                     projection = player.get('dk_fpts', player.get('Predicted_DK_Points', 8.0))
                    
#                     # 🏆 STEP 3: Get Historical Volatility Data (if available)
#                     hist_volatility = player.get('hist_volatility', projection * 0.3)
#                     hist_quality = player.get('hist_data_quality', 'limited')
#                     hist_games = player.get('hist_games_count', 0)
#                     hist_upside = player.get('hist_upside_vol', projection * 0.4)
#                     hist_downside = player.get('hist_downside_vol', projection * 0.7)
#                     hist_var = player.get('hist_var_95', projection * 0.7)
                    
#                     # 💡 STEP 4: Extract Implied Volatility from Probability Data
#                     prob_data = None
#                     if prob_columns:
#                         prob_data = self.extract_probability_distribution_from_csv(player, prob_columns)
                    
#                     if prob_data and len(prob_data) >= 2:
#                         # Calculate implied volatility metrics from probability data
#                         implied_vol_metrics = self.calculate_comprehensive_volatility_metrics(
#                             player_name, projection, prob_data
#                         )
                        
#                         # 🔀 STEP 5: Blend Historical and Implied Volatility Based on Data Quality
#                         if hist_quality == 'excellent' and hist_games >= 10:
#                             # Favor historical data for players with excellent game history
#                             blended_vol = 0.75 * hist_volatility + 0.25 * implied_vol_metrics['historical_volatility']
#                             blended_garch = 0.75 * hist_volatility + 0.25 * implied_vol_metrics['garch_forecast']
#                             data_source = "Historical-Dominant"
#                         elif hist_quality == 'good' and hist_games >= 5:
#                             # Balanced blend for good historical data
#                             blended_vol = 0.6 * hist_volatility + 0.4 * implied_vol_metrics['historical_volatility'] 
#                             blended_garch = 0.6 * hist_volatility + 0.4 * implied_vol_metrics['garch_forecast']
#                             data_source = "Balanced-Blend"
#                         else:
#                             # Favor implied volatility for limited historical data
#                             blended_vol = 0.3 * hist_volatility + 0.7 * implied_vol_metrics['historical_volatility']
#                             blended_garch = 0.3 * hist_volatility + 0.7 * implied_vol_metrics['garch_forecast']
#                             data_source = "Implied-Dominant"
                        
#                         historical_volatilities.append(blended_vol)
#                         garch_forecasts.append(blended_garch)
#                         downside_risks.append(max(hist_downside, implied_vol_metrics['downside_volatility']))
#                         upside_potentials.append(max(hist_upside, implied_vol_metrics['upside_volatility']))
#                         var_95s.append(min(hist_var, implied_vol_metrics['value_at_risk_95']))
#                         volatility_percentiles.append(implied_vol_metrics['volatility_percentile'])
#                         skewness_values.append(implied_vol_metrics.get('skewness', 0.0))
                        
#                         if idx % 20 == 0:  # Progress logging every 20 players
#                             logging.info(f"🔀 {data_source} - {player_name}: Historical={hist_volatility:.3f}, "
#                                        f"Implied={implied_vol_metrics['historical_volatility']:.3f}, "
#                                        f"Blended={blended_vol:.3f}")
                        
#                     else:
#                         # Use historical data only (or fallback if no data)
#                         if hist_games > 0:
#                             historical_volatilities.append(hist_volatility)
#                             garch_forecasts.append(hist_volatility)
#                             downside_risks.append(hist_downside)
#                             upside_potentials.append(hist_upside)
#                             var_95s.append(hist_var)
#                             volatility_percentiles.append(player.get('hist_vol_percentile', 50) / 100.0)
#                             skewness_values.append(0.0)
                            
#                             if idx % 30 == 0:
#                                 logging.info(f"📊 Historical-Only - {player_name}: Vol={hist_volatility:.3f}, Games={hist_games}")
#                         else:
#                             # Complete fallback for players without any data
#                             base_vol = projection * 0.3
                            
#                             if idx == 0:  # Log once to show we're using dk_fpts
#                                 logging.info(f"💡 Using dk_fpts column for projections (fallback example: {player_name} = {projection:.2f})")
#                             historical_volatilities.append(base_vol)
#                             garch_forecasts.append(base_vol)
#                             downside_risks.append(base_vol * 0.7)
#                             upside_potentials.append(base_vol * 1.3)
#                             var_95s.append(projection * 0.7)
#                             volatility_percentiles.append(0.5)
#                             skewness_values.append(0.0)
                        
#                 except Exception as e:
#                     logging.debug(f"Error processing {player.get('Name', 'Unknown')}: {e}")
#                     # Emergency fallback values
#                     base_vol = player['Predicted_DK_Points'] * 0.3
#                     historical_volatilities.append(base_vol)
#                     garch_forecasts.append(base_vol)
#                     downside_risks.append(base_vol * 0.7)
#                     upside_potentials.append(base_vol * 1.3)
#                     var_95s.append(player['Predicted_DK_Points'] * 0.7)
#                     volatility_percentiles.append(0.5)
#                     skewness_values.append(0.0)
            
#             # Add all calculated metrics to DataFrame
#             df['Historical_Volatility'] = historical_volatilities
#             df['GARCH_Volatility'] = garch_forecasts
#             df['Downside_Risk'] = downside_risks
#             df['Upside_Potential'] = upside_potentials
#             df['VaR_95'] = var_95s
#             df['Volatility_Percentile'] = volatility_percentiles
#             df['Skewness'] = skewness_values
            
#             # Calculate variance for optimization (using GARCH forecasts)
#             df['Variance'] = np.array(garch_forecasts) ** 2
            
#             # Calculate volatility-adjusted projections based on risk tolerance
#             df = self.calculate_volatility_adjusted_projections(df)
            
#             # Calculate additional risk metrics
#             df = self.calculate_portfolio_risk_metrics(df)
            
#             # Log summary statistics
#             avg_vol = np.mean(historical_volatilities)
#             avg_garch = np.mean(garch_forecasts)
#             logging.info(f"✅ GARCH & Volatility Analysis Complete!")
#             logging.info(f"📊 Average Historical Volatility: {avg_vol:.3f}")
#             logging.info(f"🎯 Average GARCH Forecast: {avg_garch:.3f}")
#             logging.info(f"📈 Players with probability data: {len([v for v in historical_volatilities if v != df['Predicted_DK_Points'].iloc[0] * 0.3])}")
            
#             return df
            
#         except Exception as e:
#             logging.error(f"❌ Error in volatility analysis: {e}")
#             # Complete fallback to basic calculations
#             df['Variance'] = (df['Predicted_DK_Points'] * 0.3) ** 2
#             df['GARCH_Volatility'] = df['Predicted_DK_Points'] * 0.3
#             df['Historical_Volatility'] = df['Predicted_DK_Points'] * 0.3
#             df['Volatility_Percentile'] = 0.5
...
#     def extract_probability_distribution_from_csv(self, player, prob_columns):
#         """Extract probability distribution from player data using CSV probability columns"""
#         prob_data = {}
        
#         for col in prob_columns:
#             try:
#                 # Extract threshold from column name (e.g., 'Prob_Over_15' -> 15)
#                 threshold = float(col.split('_')[-1])
#                 prob_value = player[col]
                
#                 if pd.notna(prob_value) and prob_value != '':
#                     # Handle percentage strings and convert to probability
#                     if isinstance(prob_value, str) and '%' in prob_value:
#                         prob_value = float(prob_value.replace('%', '')) / 100.0
#                     elif isinstance(prob_value, str):
#                         prob_value = float(prob_value) / 100.0 if float(prob_value) > 1 else float(prob_value)
#                     elif prob_value > 1:
#                         prob_value = prob_value / 100.0
                    
#                     # Clamp probability between 0 and 1
#                     prob_data[threshold] = max(0, min(1, prob_value))
                    
#             except (ValueError, IndexError, TypeError):
#                 continue
        
#         return prob_data if prob_data else None
    
#     def calculate_comprehensive_volatility_metrics(self, player_name, projection, prob_data):
#         """Calculate comprehensive volatility metrics using GARCH and probability analysis"""
        
#         # Create synthetic return distribution from probabilities
#         synthetic_returns = self.create_synthetic_returns_from_probabilities(projection, prob_data)
        
#         # Calculate basic volatility measures
#         historical_vol = np.std(synthetic_returns) if len(synthetic_returns) > 1 else projection * 0.3
        
#         # GARCH volatility forecast
#         garch_forecast = self.calculate_garch_forecast_from_returns(synthetic_returns, player_name)
        
#         # Probability-based volatility
#         prob_volatility = self.calculate_probability_based_volatility(prob_data, projection)
        
#         # Upside/Downside volatility
#         upside_vol, downside_vol = self.calculate_upside_downside_volatility(synthetic_returns, projection)
        
#         # Risk metrics
#         var_95 = np.percentile(synthetic_returns, 5) if len(synthetic_returns) > 0 else projection * 0.7
        
#         # Simple skewness calculation
#         if len(synthetic_returns) > 2:
#             mean_val = np.mean(synthetic_returns)
#             std_val = np.std(synthetic_returns)
#             if std_val > 0:
#                 skewness = np.mean(((synthetic_returns - mean_val) / std_val) ** 3)
#             else:
#                 skewness = 0.0
#         else:
#             skewness = 0.0
        
#         # Volatility percentile (simplified ranking)
#         vol_percentile = min(0.95, max(0.05, historical_vol / (projection * 0.5)))
        
#         return {
#             'historical_volatility': historical_vol,
#             'garch_forecast': garch_forecast,
#             'probability_volatility': prob_volatility,
...
#     def create_synthetic_returns_from_probabilities(self, projection, prob_data):
#         """Create synthetic return distribution from probability data"""
        
#         if not prob_data:
#             return np.random.normal(projection, projection * 0.3, 1000)
        
#         # Sort thresholds and probabilities
#         thresholds = sorted(prob_data.keys())
#         synthetic_points = []
        
#         # Generate synthetic data based on probability distribution
#         for i, threshold in enumerate(thresholds):
#             prob = prob_data[threshold]
            
#             if i == 0:
#                 # Below first threshold
#                 num_samples = int((1 - prob) * 100)
#                 if num_samples > 0:
#                     samples = np.random.normal(threshold * 0.7, threshold * 0.2, num_samples)
#                     synthetic_points.extend(samples)
            
#             if i < len(thresholds) - 1:
#                 # Between thresholds
#                 next_threshold = thresholds[i + 1]
#                 next_prob = prob_data[next_threshold]
#                 prob_between = max(0, prob - next_prob)
                
#                 num_samples = int(prob_between * 100)
#                 if num_samples > 0:
#                     samples = np.random.uniform(threshold, next_threshold, num_samples)
#                     synthetic_points.extend(samples)
#             else:
#                 # Above highest threshold
#                 num_samples = int(prob * 100)
#                 if num_samples > 0:
#                     samples = np.random.normal(threshold * 1.3, threshold * 0.3, num_samples)
#                     synthetic_points.extend(samples)
        
#         return np.array(synthetic_points) if synthetic_points else np.random.normal(projection, projection * 0.3, 100)
    
#     def calculate_garch_forecast_from_returns(self, returns, player_name):
#         """Calculate GARCH volatility forecast from synthetic returns"""
...
#             if len(returns) < 50:
#                 return np.std(returns) if len(returns) > 1 else 0.1
            
#             # Calculate returns from synthetic data
#             if len(returns) < 2:
#                 return 0.1
                
#             price_returns = np.diff(returns) / returns[:-1] * 100
            
#             if len(price_returns) < 30:
#                 return np.std(price_returns) / 100 if len(price_returns) > 1 else 0.1
            
#             # Fit GARCH(1,1) model
#             model = arch_model(price_returns, vol='Garch', p=1, q=1, rescale=False)
#             fitted_model = model.fit(disp='off', show_warning=False)
            
#             # Forecast volatility
#             forecast = fitted_model.forecast(horizon=1)
#             volatility_forecast = np.sqrt(forecast.variance.iloc[-1, 0]) / 100
            
#             logging.debug(f"🎯 GARCH forecast for {player_name}: {volatility_forecast:.4f}")
#             return volatility_forecast
            
#         except ImportError:
#             logging.debug("ARCH package not available - using historical volatility")
#             return np.std(returns) if len(returns) > 1 else 0.1
#         except Exception as e:
#             logging.debug(f"GARCH model failed for {player_name}: {e}")
#             return np.std(returns) if len(returns) > 1 else 0.1
    
#     def calculate_probability_based_volatility(self, prob_data, projection):
#         """Calculate volatility based on probability distribution shape"""
        
#         if not prob_data:
#             return projection * 0.3
        
#         # Calculate expected value and variance from probability distribution
#         thresholds = sorted(prob_data.keys())
#         expected_value = 0.0
#         variance = 0.0
...
#                 next_threshold = thresholds[i + 1]
#                 next_prob = prob_data[next_threshold]
#                 value = (threshold + next_threshold) / 2
#                 weight = max(0, prob - next_prob)
#             else:
#                 value = threshold * 1.2
#                 weight = prob
            
#             expected_value += value * weight
#             variance += (value ** 2) * weight
        
#         variance = max(0, variance - expected_value ** 2)
#         volatility = np.sqrt(variance)
        
#         return volatility
    
#     def calculate_upside_downside_volatility(self, returns, target):
...
#         upside_vol = np.std(upside_returns) if len(upside_returns) > 1 else 0.0
#         downside_vol = np.std(downside_returns) if len(downside_returns) > 1 else 0.0
        
#         return upside_vol, downside_vol
    
#     def calculate_volatility_adjusted_projections(self, df):
#         """Calculate volatility-adjusted projections based on risk tolerance"""
        
#         # Get risk tolerance from UI (default to medium if not set)
#         risk_tolerance = getattr(self, 'risk_tolerance', 'medium')
        
#         # Risk tolerance parameters
#         risk_params = {
#             'conservative': {'vol_penalty': 2.0, 'downside_weight': 3.0, 'stability_bonus': 1.5},
#             'medium': {'vol_penalty': 1.0, 'downside_weight': 1.5, 'stability_bonus': 1.0},
#             'aggressive': {'vol_penalty': 0.5, 'downside_weight': 0.8, 'stability_bonus': 0.5}
#         }
...
#         for idx, player in df_copy.iterrows():
#             original_proj = player['Predicted_DK_Points']
            
#             # Volatility adjustment
#             vol_penalty = player.get('Historical_Volatility', original_proj * 0.3) * params['vol_penalty']
#             downside_penalty = player.get('Downside_Risk', original_proj * 0.2) * params['downside_weight']
            
#             # Stability bonus for low volatility players
#             vol_percentile = player.get('Volatility_Percentile', 0.5)
#             if vol_percentile < 0.3:  # Bottom 30% volatility
#                 stability_bonus = original_proj * 0.05 * params['stability_bonus']
#             else:
#                 stability_bonus = 0
            
#             # GARCH-based adjustment
#             garch_vol = player.get('GARCH_Volatility', original_proj * 0.3)
#             hist_vol = player.get('Historical_Volatility', original_proj * 0.3)
#             garch_adjustment = (garch_vol - hist_vol) * 10
            
#             # Final adjusted projection
#             adjusted_proj = (original_proj 
#                            - vol_penalty 
#                            - downside_penalty 
#                            + stability_bonus 
#                            + garch_adjustment)
            
#             # Ensure projection doesn't go negative or too extreme
#             adjusted_proj = max(adjusted_proj, original_proj * 0.3)
#             adjusted_proj = min(adjusted_proj, original_proj * 1.7)
            
#             df_copy.at[idx, 'Volatility_Adjusted_Proj'] = adjusted_proj
#             df_copy.at[idx, 'Vol_Adjustment'] = adjusted_proj - original_proj
        
#         # Update Predicted_DK_Points to use volatility-adjusted projections
#         if 'Volatility_Adjusted_Proj' in df_copy.columns:
#             df_copy['Predicted_DK_Points'] = df_copy['Volatility_Adjusted_Proj']
        
#         logging.info(f"✅ Generated volatility-adjusted projections with {risk_tolerance} risk tolerance")
#         return df_copy
    
#     def calculate_portfolio_risk_metrics(self, df):
#         """Calculate additional portfolio-level risk metrics"""
        
#         # Add risk-adjusted value metric
#         df['Risk_Adjusted_Value'] = df['Predicted_DK_Points'] / (df['Salary'] / 1000) / df.get('Historical_Volatility', 1)
        
#         # Add Sharpe-like ratio for DFS
#         df['DFS_Sharpe_Ratio'] = df['Predicted_DK_Points'] / df.get('Historical_Volatility', 1)
        
#         # Add volatility rank within position
#         for pos in df['Pos'].unique():
#             pos_mask = df['Pos'] == pos
#             if pos_mask.sum() > 1:
#                 df.loc[pos_mask, 'Volatility_Rank'] = df.loc[pos_mask, 'Historical_Volatility'].rank(pct=True)
#             else:
#                 df.loc[pos_mask, 'Volatility_Rank'] = 0.5
