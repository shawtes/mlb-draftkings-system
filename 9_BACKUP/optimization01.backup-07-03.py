...
class FantasyBaseballApp(QMainWindow):
    ...
    def get_min_salary_constraint(self):
        """Get the minimum salary constraint from the UI input with enhanced validation"""
        try:
            min_salary_text = self.min_salary_input.text().strip()
            if not min_salary_text:
                logging.info("Using default minimum salary constraint: $45,000")
            ...
            if min_salary < 0:
                logging.warning("Minimum salary cannot be negative, setting to 0")
                self.min_salary_input.setText("0")
                min_salary = 0
            elif min_salary > SALARY_CAP:
                logging.warning(f"Minimum salary cannot exceed salary cap ({SALARY_CAP}), setting to cap")
                self.min_salary_input.setText(str(SALARY_CAP))
                min_salary = SALARY_CAP
            elif min_salary < 30000:
                logging.info(f"Low minimum salary ({min_salary}) - lineups may include many cheap players")
            elif min_salary > 47000:
                logging.info(f"High minimum salary ({min_salary}) - lineups will be forced to use premium players")
            
            # Log the final minimum salary for user awareness
            if min_salary > 0:
                logging.info(f"💰 Minimum salary constraint set to: ${min_salary:,}")
            else:
                logging.info("💰 No minimum salary constraint (allowing any budget)")
            
            return min_salary
            
        except ValueError:
            logging.warning(f"Invalid minimum salary value: '{self.min_salary_input.text()}' - using default $45,000")
            self.min_salary_input.setText("45000")  # Reset to default
    ...
    def create_status_message(self, total_lineups, requested_lineups, min_unique, has_risk_info, results):
        """Create comprehensive status message including salary constraints"""
        unique_msg = f" (Min unique: {min_unique})" if min_unique > 0 else ""
        risk_msg = ""
        salary_msg = ""
        
        # Add minimum salary information if constraint is active
        if hasattr(self, 'min_salary') and self.min_salary > 0:
            salary_msg = f" | 💰 Min Salary: ${self.min_salary:,}"
        ...
        if total_lineups < requested_lineups:
            return f"⚠️ Generated {total_lineups}/{requested_lineups} lineups{unique_msg}{salary_msg}{risk_msg}"
        else:
            return f"✅ Generated {total_lineups} lineups{unique_msg}{salary_msg}{risk_msg}"
    
    def show_optimization_feedback(self, total_lineups, requested_lineups, min_unique, has_risk_info):
        """Show detailed optimization feedback dialog including salary constraints"""
        risk_text = "\n🔥 Risk-adjusted optimization was used" if has_risk_info else ""
        
        # Add minimum salary constraint information
        salary_text = ""
        if hasattr(self, 'min_salary') and self.min_salary > 0:
            salary_text = f"\n💰 Minimum salary constraint: ${self.min_salary:,}"
            if total_lineups < requested_lineups * 0.5:  # If severely limited
                salary_text += "\n   (High minimum salary may limit lineup combinations)"
        ...
        QMessageBox.information(
            ...
            f"{salary_text}"
            f"{risk_text}\n\n"
            f"💡 Tip: Try reducing min unique constraint, lowering minimum salary, or adjusting stack settings for more lineups."
        )
    ...
    def filter_lineups_by_uniqueness(self, results, min_unique):
        ...
        if len(filtered_results) < 5 and len(results) >= 5:
            ...
            filtered_results = {}
            for i, (key, lineup_data) in enumerate(emergency_sorted[:5]):
