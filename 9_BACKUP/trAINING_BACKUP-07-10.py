...
def inspect_data_structure(file_path, n_rows=1000):
    """
    Quickly inspects the structure of a CSV file by reading a sample.
    Returns the data types of the columns.
    """
    import pandas as pd
    print(f"Inspecting first {n_rows} rows of {file_path}...")
    sample = pd.read_csv(file_path, nrows=n_rows, low_memory=False)
...
HARDCODED_OPTIMAL_PARAMS = {
    ...
    'model__max_features': None,  # Optional, can be removed if not needed
    # Add more valid GradientBoostingRegressor params as needed
}
...
base_models = [
    ...
    ('bag', BaggingRegressor(n_estimators=30, random_state=42, n_jobs=-1)),
]
...
def save_feature_importance(pipeline, output_csv_path, output_plot_path):
    ...
    # Extract feature importances directly from GradientBoostingRegressor
    feature_importances = None
    feature_source = None
    try:
        if hasattr(model, 'feature_importances_'):
            feature_importances = model.feature_importances_
            feature_source = 'GradientBoostingRegressor'
        elif hasattr(model, 'coef_'):
            feature_importances = np.abs(model.coef_)
            feature_source = 'Linear Model (coef_)'
    except Exception as e:
        print(f"Error retrieving feature importances: {e}")

    if feature_importances is None:
        raise ValueError("Could not retrieve feature importances from the model. Make sure the model supports feature_importances_ or coef_.")
...
def load_or_create_label_encoders(df, name_encoder_path, team_encoder_path):
    """Stub for label encoder loader/creator."""
...
def load_or_create_scaler(df, numeric_features, scaler_path):
    """Stub for scaler loader/creator."""
    # TODO: Implement actual logic
    return None

def clean_infinite_values(df):
    """Stub for cleaning infinite values."""
    # TODO: Implement actual logic
    return df
...
if __name__ == "__main__":
    ...
    print(f"  • XGBoost Estimators: {HARDCODED_OPTIMAL_PARAMS['model__n_estimators']:,}")
    ...
    name_encoder_path = os.path.join(script_directory, 'label_encoder_name_sep202.pkl')
    team_encoder_path = os.path.join(script_directory, 'label_encoder_team_sep202.pkl')
    scaler_path = os.path.join(script_directory, 'scaler_sep202.pkl')
    ...
    file_path = 'c:/Users/smtes/OneDrive/Documents/draftkings project/MLB_DRAFTKINGS_SYSTEM/4_DATA/filtered_data.csv'
    ...
    print("Note: This trains a single GradientBoostingRegressor model.")

    # Fit the pipeline to ensure it is fitted in scikit-learn's eyes
    ...
    model_pipeline_path = os.path.join(script_directory, 'batters_final_ensemble_model_pipeline02.pkl')
    ...
    print(f"XGBoost estimators: {HARDCODED_OPTIMAL_PARAMS.get('model__n_estimators', 'N/A')}")
    print(f"Max tree depth: {HARDCODED_OPTIMAL_PARAMS.get('model__max_depth', 'N/A')}")
    print(f"Max bins: {HARDCODED_OPTIMAL_PARAMS.get('model__max_bin', 'N/A')}")
    print(f"{'='*70}")
