...
class NBAParlayLeg:
    ...
    team: str
    position: str
    prop_type: str  # 'points', 'rebounds', 'assists', 'steals', 'blocks', 'three_pointers'
    line: float
    projection: float
    confidence: float
    hit_rate: float
...
class ParlayEnvironment(gym.Env):
    ...
    def __init__(self, training_data: pd.DataFrame, max_legs: int = 4):
        ...
        # prop_type: 0=points, 1=rebounds, 2=assists, 3=steals, 4=blocks, 5=three_pointers
        # line_multiplier: 0.5, 0.6, 0.7, 0.8, 0.9 (70% of projection = 0.7)
        
        self.n_players = len(training_data)
        self.n_prop_types = 6  # NBA has 6 prop types
    ...
    def _create_parlay_leg(self, player: pd.Series, prop_type: int, line_multiplier: int) -> NBAParlayLeg:
        """Create a parlay leg from player data and action"""
        prop_types = ['points', 'rebounds', 'assists', 'steals', 'blocks', 'three_pointers']
        ...
        if prop_type_name == 'points':
            projection = player['projected_points']
        elif prop_type_name == 'rebounds':
            projection = player['projected_rebounds']
        elif prop_type_name == 'assists':
            projection = player['projected_assists']
        elif prop_type_name == 'steals':
            projection = player['projected_steals']
        elif prop_type_name == 'blocks':
            projection = player['projected_blocks']
        else:  # three_pointers
            projection = player['projected_three_pointers']
    ...
    def _calculate_confidence(self, player: pd.Series, prop_type: str) -> float:
        ...
        if prop_type == 'points':
            accuracy = player.get('points_hit_mean', 0.7)
            consistency = 1.0 - player.get('points_accuracy_std', 0.25)
        elif prop_type == 'rebounds':
            accuracy = player.get('rebounds_hit_mean', 0.75)
            consistency = 1.0 - player.get('rebounds_accuracy_std', 0.20)
        elif prop_type == 'assists':
            accuracy = player.get('assists_hit_mean', 0.75)
            consistency = 1.0 - player.get('assists_accuracy_std', 0.20)
        elif prop_type == 'steals':
            accuracy = player.get('steals_hit_mean', 0.68)
            consistency = 1.0 - player.get('steals_accuracy_std', 0.25)
        elif prop_type == 'blocks':
            accuracy = player.get('blocks_hit_mean', 0.65)
            consistency = 1.0 - player.get('blocks_accuracy_std', 0.30)
        else:  # three_pointers
            accuracy = player.get('three_pointers_hit_mean', 0.60)
            consistency = 1.0 - player.get('three_pointers_accuracy_std', 0.35)
    ...
    def _calculate_hit_rate(self, player: pd.Series, prop_type: str, multiplier: float) -> float:
        ...
        if prop_type == 'points':
            base_hit_rate = player.get('points_hit_mean', 0.7)
        elif prop_type == 'rebounds':
            base_hit_rate = player.get('rebounds_hit_mean', 0.75)
        elif prop_type == 'assists':
            base_hit_rate = player.get('assists_hit_mean', 0.75)
        elif prop_type == 'steals':
            base_hit_rate = player.get('steals_hit_mean', 0.68)
        elif prop_type == 'blocks':
            base_hit_rate = player.get('blocks_hit_mean', 0.65)
        else:  # three_pointers
            base_hit_rate = player.get('three_pointers_hit_mean', 0.60)
