...
try:
    ...
    print("⚠️ Scipy not available - some advanced risk features disabled")
...
try:
    ...
    print("⚠️ Sklearn not available - some ML risk features disabled")
from collections import defaultdict
...
# ========== ADVANCED RISK MANAGEMENT CLASSES ==========

class GARCHRiskModel:
    """GARCH volatility modeling for advanced risk forecasting"""
    def __init__(self, p=1, q=1, lookback=100):
        self.p = p  # GARCH p parameter
        self.q = q  # GARCH q parameter
    ...
    def fit(self, returns_series):
        ...
        try:
            if len(returns_series) < 30:
                logging.warning("⚠️ Insufficient data for GARCH fitting, using defaults")
                self.fitted = False
                return
            
            # Simple GARCH(1,1) estimation using rolling volatility
            self.volatility_forecast = returns_series.rolling(window=min(30, len(returns_series)//2)).std().iloc[-1]
            if pd.isna(self.volatility_forecast):
                self.volatility_forecast = 0.2
            
            self.fitted = True
            logging.info(f"📊 GARCH model fitted with volatility forecast: {self.volatility_forecast:.4f}")
            
        except Exception as e:
            logging.warning(f"⚠️ GARCH fitting failed: {e}, using default volatility")
            self.volatility_forecast = 0.2
            self.fitted = False
    ...
    def forecast_volatility(self, steps=1):
        ...
        if self.fitted:
            ...
            return 0.2
        """Estimate GARCH volatility for risk forecasting"""
        try:
            from arch import arch_model
            
            # Fit GARCH model
            model = arch_model(returns_series, vol='GARCH', p=self.p, q=self.q)
            ...
            return {'volatility': returns_series.rolling(self.lookback).std().iloc[-1]}
    ...
    def calculate_var(self, returns, method='historical'):
        ...
        try:
            ...
            if method == 'historical':
                ...
                if SCIPY_AVAILABLE:
                    var = stats.norm.ppf(1 - self.confidence_level, mean_return, std_return)
                else:
                    # Simple approximation for 95% confidence
                    var = mean_return - 1.645 * std_return
            
            return var
            
        except Exception as e:
            logging.warning(f"⚠️ VaR calculation failed: {e}")
            return -0.05
    ...
    def calculate_expected_shortfall(self, returns):
        ...
        try:
            ...
            if len(tail_returns) > 0:
                ...
                return var * 1.3  # Approximation
                
        except Exception as e:
            logging.warning(f"⚠️ Expected Shortfall calculation failed: {e}")
            return -0.065
        ...
        if method == 'historical':
            ...
            try:
                ...
                var = returns.mean() - 2 * returns.std()
                es = var - returns.std()
...
class AdvancedRiskModel:
    ...
    def fit_factor_model(self, returns, factors):
        """Fit factor model to decompose risk"""
        try:
            if factors is None or len(factors) == 0:
                logging.warning("⚠️ No factors provided for risk model")
                return
            ...
            if SKLEARN_AVAILABLE:
                ...
                min_length = min(len(returns), len(factors))
                returns_aligned = returns.iloc[:min_length] if hasattr(returns, 'iloc') else returns[:min_length]
                factors_aligned = factors.iloc[:min_length] if hasattr(factors, 'iloc') else factors[:min_length]
                
                # Fit linear regression
                model = LinearRegression()
                model.fit(factors_aligned, returns_aligned)
                
                self.factor_loadings = model.coef_
                self.fitted = True
                logging.info(f"📊 Factor model fitted with {len(self.factor_loadings)} factors")
            else:
                # Simple correlation-based approach
                self.factor_loadings = np.ones(len(factors.columns) if hasattr(factors, 'columns') else 1) * 0.5
                self.fitted = True
                logging.info("📊 Factor model fitted with correlation approximation")
                
        except Exception as e:
            logging.warning(f"⚠️ Factor model fitting failed: {e}")
            self.fitted = False

    def create_momentum_factor(self, returns_df, lookback=60):
        """Create momentum factor from past returns"""
        momentum = returns_df.rolling(lookback).mean().mean(axis=1)
        return momentum.fillna(0)
    
    def create_volatility_factor(self, returns_df, lookback=30):
        """Create volatility factor"""
        volatility = returns_df.rolling(lookback).std().mean(axis=1)
        return volatility.fillna(volatility.mean())
    
    def calculate_factor_loadings(self, returns_df, factors):
        """Calculate factor loadings via time-series regression"""
        loadings = {}
        
        for asset in returns_df.columns:
            if asset in ['market_cap']:  # Skip non-return columns
                continue
                
            asset_returns = returns_df[asset].dropna()
            aligned_factors = factors.loc[asset_returns.index]
            
            try:
                import statsmodels.api as sm
                # Run regression: asset_returns = alpha + beta * factors + error
                X = sm.add_constant(aligned_factors)
                model = sm.OLS(asset_returns, X).fit()
                
                loadings[asset] = {
                    'market': model.params.get('market', 0),
                    'momentum': model.params.get('momentum', 0), 
                    'volatility': model.params.get('volatility', 0),
                ...
                loadings[asset] = {
                    'market': asset_returns.corr(aligned_factors['market']),
                    'momentum': asset_returns.corr(aligned_factors['momentum']),
                    'volatility': asset_returns.corr(aligned_factors['volatility']),
                    'alpha': 0,
                    'r_squared': 0.5
                }
    ...
    def calculate_specific_risk(self, returns_df, factors):
        """Calculate specific (idiosyncratic) risk for each asset"""
        specific_risk = {}
        
        for asset in returns_df.columns:
            if asset in ['market_cap']:
                continue
                
            asset_returns = returns_df[asset].dropna()
            # Simple specific risk calculation
            total_variance = asset_returns.var()
            systematic_variance = total_variance * 0.6  # Assume 60% systematic
            specific_risk[asset] = total_variance - systematic_variance
...
class LiquidityRiskManager:
    ...
    def calculate_liquidity_metrics(self, df_players):
        """Calculate liquidity risk metrics for each player/security"""
        ...
        for _, player in df_players.iterrows():
            ...
            # Mock liquidity calculations (replace with real data)
            # 1. Average Daily Volume (if available)
            avg_volume = player.get('Avg_Volume', 1000000)  # Default volume
            
            # 2. Bid-Ask Spread proxy (inversely related to salary for DFS)
            spread_proxy = 1000 / max(player['Salary'], 3000)  # Lower salary = higher spread
            
            # 3. Market Impact Cost (based on position size relative to volume)
            position_size = 50000 / len(df_players)  # Assume equal allocation
            impact_cost = (position_size / avg_volume) * 0.01  # 1% per volume ratio
            
            # 4. Liquidity Score (higher = more liquid)
            liquidity_score = 1 / (1 + spread_proxy + impact_cost)
            
            liquidity_metrics[name] = {
                'avg_volume': avg_volume,
                'spread_proxy': spread_proxy,
                'impact_cost': impact_cost,
                'liquidity_score': liquidity_score,
                'risk_adjustment': max(0.8, liquidity_score)  # Minimum 80% weighting
            }
            
        return liquidity_metrics
    
    def adjust_positions_for_liquidity(self, lineup, liquidity_metrics):
        """Adjust lineup based on liquidity constraints"""
        
        adjusted_lineup = lineup.copy()
        
        for idx, player in lineup.iterrows():
            name = player['Name']
            if name in liquidity_metrics:
                # Reduce effective points by liquidity risk
                risk_adj = liquidity_metrics[name]['risk_adjustment']
                adjusted_lineup.at[idx, 'Liquidity_Adjusted_Points'] = (
                    player['Predicted_DK_Points'] * risk_adj
                )
        
        return adjusted_lineup

class EnhancedKellyCriterion:
    """Enhanced Kelly Criterion with risk controls"""
    def __init__(self, max_kelly_fraction=0.25):
        self.max_kelly_fraction = max_kelly_fraction
        
    def calculate_kelly_sizing(self, expected_return, win_probability, 
                             odds_received, volatility=None):
        """Enhanced Kelly sizing with risk controls"""
        
        # Standard Kelly: f = (bp - q) / b
        # where b = odds, p = win prob, q = lose prob = 1-p
        
        if odds_received <= 0 or win_probability <= 0 or win_probability >= 1:
            return 0
            
        lose_probability = 1 - win_probability
        kelly_fraction = (odds_received * win_probability - lose_probability) / odds_received
        
        # Apply volatility adjustment if available
        if volatility and volatility > 0:
            # Reduce Kelly fraction for higher volatility
            volatility_adjustment = 1 / (1 + volatility)
            kelly_fraction *= volatility_adjustment
            
        # Apply maximum Kelly limit (risk management)
        ...
        return {
            ...
            'risk_limited': kelly_fraction == self.max_kelly_fraction
        }
    ...
    def calculate_optimal_position_size(self, expected_return, volatility, bankroll):
        ...
        try:
            ...
            # Assuming risk-free rate ≈ 0 for DFS
            if volatility <= 0:
                return bankroll * 0.02  # Conservative default
            
            kelly_fraction = expected_return / (volatility ** 2)
            
            # Apply maximum Kelly fraction limit
            kelly_fraction = min(kelly_fraction, self.max_kelly_fraction)
            kelly_fraction = max(kelly_fraction, 0.001)  # Minimum 0.1%
            
            optimal_size = bankroll * kelly_fraction
            
            # Ensure reasonable bounds
            optimal_size = min(optimal_size, bankroll * 0.25)  # Never more than 25%
            optimal_size = max(optimal_size, bankroll * 0.01)   # At least 1%
            
            return optimal_size
            
        except Exception as e:
            logging.warning(f"⚠️ Kelly calculation failed: {e}")
            return bankroll * 0.02  # 2% default
...
class PortfolioRiskAnalyzer:
    ...
    def decompose_portfolio_risk(self, portfolio_weights, returns_df):
        ...
        # Simplified risk decomposition for DFS
        total_risk = 0
        factor_risk = 0
        specific_risk = 0
        ...
        for asset, weight in portfolio_weights.items():
            if asset in returns_df.columns:
                ...
                total_risk += weight * weight * asset_variance
                
        return {
            'total_variance': total_risk,
            'total_volatility': np.sqrt(max(total_risk, 0.0001)),
            'factor_risk': factor_risk,
            'specific_risk': specific_risk,
            'factor_contribution': 0.6,  # Assume 60% factor risk
            'specific_contribution': 0.4  # Assume 40% specific risk
        }

# ========== END RISK MANAGEMENT CLASSES ==========
...
def optimize_single_lineup(args):
    ...
    if stack_type == "No Stacks":
        ...
        if not valid_teams:
            ...
            # ENHANCED TEAM SELECTION FOR DIVERSITY
            # Instead of using all teams or just one team, randomly select from available teams
            # This creates diverse lineups across your selected teams
            ...
            if len(valid_teams) == 1:
                ...
                # CRITICAL FIX: For multiple teams, randomly select 1-2 teams per lineup for diversity
                # This ensures we get different combinations instead of always the same optimal team
                
                # Randomly select 1-2 teams from your selected teams for THIS specific lineup
                num_teams_to_use = min(2, len(valid_teams))  # Use 1-2 teams max per lineup
                selected_teams_for_this_lineup = random.sample(valid_teams, 
                                                             random.randint(1, num_teams_to_use))
                
                logging.info(f"🎯 DIVERSITY: Selected teams for THIS lineup: {selected_teams_for_this_lineup} from available: {valid_teams}")
                
                # Create binary variables for the selected teams
                team_binary_vars = {}
                for team in selected_teams_for_this_lineup:
                    team_binary_vars[team] = pulp.LpVariable(f"use_team_{team}_{stack_size}", cat='Binary')
                
                # At least one of the selected teams must be used
                ...
                logging.info(f"✅ ENFORCING: Must have at least {stack_size} players from ANY of: {selected_teams_for_this_lineup}")
            
            # Log final enforcement summary
            logging.info(f"📊 STACK CONSTRAINT SUMMARY: {stack_size}-stack will be enforced")
    else:
        ...
        for i, size in enumerate(stack_sizes):
            ...
            # ENHANCED TEAM SELECTION FOR DIVERSITY (Complex Stacks)
            # Instead of using all teams, randomly select from available teams for diversity
            ...
            if len(valid_teams) == 1:
                ...
                # CRITICAL FIX: For multiple teams, randomly select fewer teams per lineup for diversity
                # This ensures we get different combinations instead of always the same optimal team
                ...
                logging.info(f"🎯 DIVERSITY ({size}-stack): Selected teams for THIS lineup: {selected_teams_for_this_lineup} from available: {valid_teams}")
                ...
                logging.info(f"✅ ENFORCING: Must have at least {size} players from ANY of: {selected_teams_for_this_lineup}")
            
            # Log final enforcement summary
            logging.info(f"📊 STACK CONSTRAINT SUMMARY: {size}-stack will be enforced")
...
class OptimizationWorker(QThread):
    ...
    def __init__(self, df_players, salary_cap, position_limits, included_players, stack_settings, min_exposure, max_exposure, min_points, monte_carlo_iterations, num_lineups, team_selections, min_unique=0, bankroll=1000, risk_tolerance='medium', disable_kelly=False, min_salary=None, use_advanced_quant=False, advanced_quant_params=None):
        ...
        # Initialize enhanced risk management models
        ...
        # Calculate liquidity metrics if data is available
        if len(df_players) > 10:  # Need minimum data for risk models
            self.setup_enhanced_risk_models(df_players)
            
        logging.info("🔥 Enhanced risk management models initialized with quantitative book methods")

    def run(self):
        logging.debug("OptimizationWorker: Starting optimization")
        
        # Check if enhanced risk management is available
        has_enhanced_risk = (hasattr(self, 'garch_model') and 
                           hasattr(self, 'var_calculator') and 
                           hasattr(self, 'risk_model'))
        
        if has_enhanced_risk:
            logging.info("🔥 Using ENHANCED risk management with quantitative finance methods")
            results, team_exposure, stack_exposure = self.optimize_with_enhanced_risk_management()
        else:
            logging.debug("📊 Using standard optimization")
    ...
    def optimize_lineups(self):
        ...
        # Traditional optimization - NOW WITH SEQUENTIAL DIVERSITY for multiple lineups
        logging.info("📊 Using traditional optimization with SEQUENTIAL DIVERSITY")
        
        # For multiple lineups (>1), always use sequential optimization to ensure diversity
        if self.num_lineups > 1:
            logging.info("🔥 Multiple lineups requested - using SEQUENTIAL optimization for true diversity")
            return self.optimize_lineups_sequential(df_filtered, team_exposure, stack_exposure)
        
        # For single lineup, use traditional approach
        logging.info("📊 Single lineup requested - using traditional optimization")
            ...
            for future in concurrent.futures.as_completed(futures):
                try:
                    ...
                    if not lineup.empty:
                        ...
                        break  # Only need one lineup for single lineup request
                except Exception as e:
    ...
    def optimize_lineups_with_risk_management(self, df_filtered, team_exposure, stack_exposure):
        """
        Advanced lineup optimization using SEQUENTIAL optimization with player exclusion for TRUE diversity
        """
        logging.info("🔥 Starting SEQUENTIAL optimization with player exclusion for maximum diversity")
        
        results = {}
        lineup_candidates = []
        used_players_across_lineups = set()  # Track players used across all lineups
        
        # Step 1: Generate truly diverse lineups using SEQUENTIAL OPTIMIZATION
        # For team combinations, generate num_lineups for THIS specific combination
        target_lineups = self.num_lineups  # Changed from self.num_lineups * len(self.stack_settings)
        
        logging.info(f"🎯 Generating {target_lineups} SEQUENTIAL lineups with player exclusion constraints")
        
        # Main lineup generation loop with dynamic exclusion recovery
        failed_attempts = 0
        max_failed_attempts = 1  # Trigger recovery immediately on first failure for constrained combinations
        ...
        for lineup_num in range(target_lineups):
            ...
            for stack_type in self.stack_settings:
                try:
                    # Generate lineup with exclusion constraints
                    lineup, actual_stack = self.optimize_single_lineup_with_exclusions(
                        df_filtered.copy(), 
                        stack_type, 
                        used_players_across_lineups,
                        lineup_num
                    )
                    
                    if lineup is not None and not lineup.empty:
                        score = lineup['Predicted_DK_Points'].sum()
                        if score > best_score:
                            best_lineup = lineup
                            best_score = score
                            best_stack_type = actual_stack
                            
                except Exception as e:
                    logging.error(f"Error generating lineup {lineup_num} with stack {stack_type}: {str(e)}")
                    continue
            
            # Handle failed lineup generation with dynamic exclusion reduction
            if best_lineup is None:
                failed_attempts += 1
                logging.warning(f"❌ Failed to generate lineup {lineup_num+1}/{target_lineups} (attempt {failed_attempts})")
                
                # If we've failed, immediately try exclusion reduction for constrained combinations
                if failed_attempts >= max_failed_attempts and len(used_players_across_lineups) > 3:
                    # Progressive exclusion reduction - more aggressive for constrained combinations
                    if failed_attempts == 1:
                        # First failure recovery: Keep only 30% of exclusions (more aggressive)
                        exclusion_list = list(used_players_across_lineups)
                        keep_count = max(3, len(exclusion_list) // 3)
                        used_players_across_lineups = set(exclusion_list[-keep_count:])
                        logging.warning(f"🔄 RECOVERY: Reduced exclusions to {len(used_players_across_lineups)} players (70% reduction)")
                    elif failed_attempts == 2:
                        # Second failure recovery: Keep only 15% of exclusions
                        exclusion_list = list(used_players_across_lineups)
                        keep_count = max(2, len(exclusion_list) // 6)
                        used_players_across_lineups = set(exclusion_list[-keep_count:])
                        logging.warning(f"🔄 RECOVERY: Reduced exclusions to {len(used_players_across_lineups)} players (85% reduction)")
                    elif failed_attempts >= 3:
                        # Final recovery: Clear almost all exclusions
                        used_players_across_lineups = set()
                        logging.warning(f"🔄 RECOVERY: Cleared all exclusions to ensure lineup generation")
                    
                    # Retry the current lineup with reduced exclusions
                    for stack_type in self.stack_settings:
                        try:
                            lineup, actual_stack = self.optimize_single_lineup_with_exclusions(
                                df_filtered.copy(), 
                            ...
                            if lineup is not None and not lineup.empty:
                                ...
                                if score > best_score:
                                    ...
                                    best_score = score
                                    best_stack_type = actual_stack
                                    logging.info(f"✅ RECOVERY SUCCESS: Generated lineup {lineup_num+1} after exclusion reduction")
                                    break
                                    
                        except Exception as e:
                            continue
            
            # Add the best lineup found for this position
            if best_lineup is not None:
                failed_attempts = 0  # Reset failure counter on success
                lineup_data = self.calculate_lineup_metrics(best_lineup, best_stack_type)
                lineup_candidates.append(lineup_data)
                
                # Add players to exclusion set (ADAPTIVE exclusion to prevent over-constraining)
                new_players = set(best_lineup['Name'].tolist())
                used_players_across_lineups.update(new_players)
                
                # ADAPTIVE EXCLUSION: Reduce exclusions if we're having trouble generating lineups
                ...
                # If we're approaching the limit, trim the exclusion set
                if len(used_players_across_lineups) > exclusion_limit:
                    # Keep only the most recent players in exclusion set
                    used_players_across_lineups = new_players.copy()
                    logging.info(f"🔄 Adaptive exclusion: Reset to {len(used_players_across_lineups)} players after lineup {lineup_num+1}")
                
                # Prevent over-exclusion by clearing some players after every few lineups
                elif lineup_num > 0 and lineup_num % 3 == 0:
                    # Keep only the most recent lineup's players in exclusion set
                    used_players_across_lineups = new_players.copy()
                    logging.info(f"🔄 Cleared player exclusions after lineup {lineup_num} to prevent over-constraining")
                
                logging.info(f"✅ Generated lineup {lineup_num+1}/{target_lineups}: {best_score:.2f} points, {len(used_players_across_lineups)} players excluded")
            else:
                logging.warning(f"❌ FINAL FAILURE: Could not generate lineup {lineup_num+1}/{target_lineups} even after exclusion reduction")
        
        logging.info(f"📊 Generated {len(lineup_candidates)} diverse lineup candidates using sequential optimization")
        ...
        # Since we're using sequential optimization with exclusions, we already have diverse lineups
        # Skip the redundant min_unique filtering that was causing the "FILTERED OUT" issue
        ...
        logging.info(f"🎯 Sequential optimization complete. Selected {len(results)} truly diverse lineups")
        return results, team_exposure, stack_exposure

    def optimize_lineups_sequential(self, df_filtered, team_exposure, stack_exposure):
        """
        Sequential lineup optimization WITHOUT risk engine - ensures diverse lineups through player exclusion
        """
        logging.info("🔥 Starting SEQUENTIAL optimization for diverse lineups (no risk engine)")
        
        results = {}
        used_players_across_lineups = set()  # Track players used across all lineups
        
        # Generate lineups sequentially with player exclusion
        target_lineups = self.num_lineups
        logging.info(f"🎯 Generating {target_lineups} SEQUENTIAL lineups with player exclusion constraints")
        
        # Main lineup generation loop
        failed_attempts = 0
        ...
        for lineup_num in range(target_lineups):
            ...
            if best_lineup is None:
                ...
                # Progressive exclusion reduction for recovery
                if failed_attempts >= max_failed_attempts and len(used_players_across_lineups) > 3:
                    if failed_attempts == 1:
                        # First failure recovery: Keep only 30% of exclusions
                        exclusion_list = list(used_players_across_lineups)
                        keep_count = max(3, len(exclusion_list) // 3)
                        used_players_across_lineups = set(exclusion_list[-keep_count:])
                        logging.warning(f"🔄 RECOVERY: Reduced exclusions to {len(used_players_across_lineups)} players (70% reduction)")
            ...
            if best_lineup is not None:
                ...
                # Create lineup data in expected format (simplified without risk metrics)
                total_points = best_lineup['Predicted_DK_Points'].sum()
                results[len(results)] = {
                    'total_points': total_points, 
                    'lineup': best_lineup
                }
                ...
                # Track exposure
                for team in best_lineup['Team'].unique():
                    team_exposure[team] += 1
                stack_exposure[best_stack_type] += 1
                
                logging.info(f"✅ Generated lineup {lineup_num+1}/{target_lineups}: {best_score:.2f} points, {len(used_players_across_lineups)} players excluded")
            else:
                logging.warning(f"❌ FINAL FAILURE: Could not generate lineup {lineup_num+1}/{target_lineups} even after exclusion reduction")
        
        logging.info(f"🎯 Sequential optimization complete. Generated {len(results)} diverse lineups")
        return results, team_exposure, stack_exposure

    def optimize_single_lineup_with_exclusions(self, df, stack_type, excluded_players, lineup_num):
        """
        Optimize a single lineup with player exclusion constraints for true diversity
        """
        import random
        import numpy as np
        import time
        ...
        logging.debug(f"Generating lineup #{lineup_num+1} with {len(excluded_players)} excluded players")
        
        # Ensure truly random noise for each lineup
        seed_value = int(time.time() * 1000000) % 2147483647 + os.getpid() + lineup_num
        ...
        # SMART EXCLUSION: Consider team requirements when excluding players
        if excluded_players:
            original_count = len(df)
            
            # If we have team selections, be smarter about exclusions
            if hasattr(self, 'team_selections') and self.team_selections:
                # Calculate required teams and their minimum player counts
                team_requirements = {}
                for stack_size, teams in self.team_selections.items():
                    for team in teams:
                        if team not in team_requirements:
                            team_requirements[team] = 0
                        team_requirements[team] = max(team_requirements[team], stack_size)
                
                # For each required team, ensure we don't exclude too many players
                smart_excluded = []
                for player in excluded_players:
                    ...
                    if not player_row.empty:
                        ...
                        # If this player is from a required team, check if we can afford to exclude them
                        if player_team in team_requirements:
                            # Count current available players for this team (excluding pitchers)
                            team_players = df[(df['Team'] == player_team) & (~df['Position'].str.contains('P', na=False))]
                            already_excluded_from_team = [p for p in smart_excluded 
                                                        if not df[df['Name'] == p].empty and 
                                                        df[df['Name'] == p].iloc[0]['Team'] == player_team]
                            ...
                            if available_count > required_count:
                                ...
                                logging.debug(f"Smart exclusion: {player} from {player_team} (available: {available_count}, need: {required_count})")
                            else:
                                logging.debug(f"Smart retention: Keeping {player} from {player_team} to meet team requirements (available: {available_count}, need: {required_count})")
                        else:
                            # Not from a required team, safe to exclude
                            smart_excluded.append(player)
                
                df = df[~df['Name'].isin(smart_excluded)]
                logging.info(f"Smart exclusion: Excluded {len(smart_excluded)}/{len(excluded_players)} players considering team requirements, {len(df)} players remaining")
            else:
                # No team requirements, use standard exclusion
                df = df[~df['Name'].isin(excluded_players)]
                logging.debug(f"Standard exclusion: Excluded {original_count - len(df)} players, {len(df)} players remaining")
        
        # Verify we still have enough players per position
        position_counts = {}
        for position in POSITION_LIMITS.keys():
            position_counts[position] = len(df[df['Position'].str.contains(position, na=False)])
            if position_counts[position] < POSITION_LIMITS[position]:
                logging.warning(f"Position {position} has only {position_counts[position]} players (need {POSITION_LIMITS[position]})")
                # If we don't have enough players due to exclusions, allow some back
                if excluded_players:
                    excluded_for_position = [p for p in excluded_players 
                                           if p in df[df['Position'].str.contains(position, na=False)]['Name'].tolist()]
                    if excluded_for_position:
                        # Allow the lowest projected player back for this position
                        original_df = df.copy()  # Save excluded state
                        temp_df = df.copy()
                        # Add back ALL players temporarily to find the worst ones to allow back
                        temp_df = pd.concat([temp_df, df[df['Name'].isin(excluded_for_position)]], ignore_index=True)
                        worst_players = temp_df[temp_df['Position'].str.contains(position, na=False)].nsmallest(
                            POSITION_LIMITS[position] - position_counts[position], 'Predicted_DK_Points')
                        df = pd.concat([original_df, worst_players], ignore_index=True)
                        logging.info(f"Allowed back {len(worst_players)} worst {position} players to meet minimum requirements")
        
        # Create the optimization problem
        problem = pulp.LpProblem(f"DFS_Lineup_Optimization_{lineup_num}", pulp.LpMaximize)
        
        # Create binary variables for each player
        player_vars = {}
        for idx in df.index:
            player_vars[idx] = pulp.LpVariable(f"player_{idx}_{lineup_num}", cat='Binary')
        
        # Add moderate noise for diversity (less aggressive since we have exclusions)
        diversity_factor = random.uniform(0.05, 0.15)  # Reduced noise since exclusions provide main diversity
        noise = np.random.normal(1.0, diversity_factor, len(df))
        
        # Boost some random players slightly
        if len(df) > 3:
            player_boost = np.random.choice(df.index, size=random.randint(1, 3), replace=False)
            for idx in player_boost:
                noise[df.index.get_loc(idx)] *= random.uniform(1.05, 1.15)
        ...
        if self.min_salary and self.min_salary > 0:
            problem += pulp.lpSum([df.at[idx, 'Salary'] * player_vars[idx] for idx in df.index]) >= self.min_salary
        
        # Position constraints
        for position, limit in POSITION_LIMITS.items():
            available_for_position = [idx for idx in df.index if position in df.at[idx, 'Position']]
            if len(available_for_position) < limit:
                logging.error(f"Insufficient players for {position}: need {limit}, have {len(available_for_position)}")
                return pd.DataFrame(), stack_type
            problem += pulp.lpSum([player_vars[idx] for idx in df.index if position in df.at[idx, 'Position']]) == limit

        # Add stacking constraints - ENHANCED to handle team combinations properly
        if stack_type != "No Stacks":
            logging.debug(f"Processing stack_type: {stack_type}")
            logging.debug(f"Team selections available: {self.team_selections}")
            
            # Handle team combination constraints (like CHC(5) + BOS(2))
            team_stack_constraints_applied = False
            
            # Check if team_selections has specific team-stack mappings for combinations
                ...
                for stack_size, teams in self.team_selections.items():
                    if isinstance(stack_size, (int, str)) and str(stack_size).isdigit():
                        ...
                        if isinstance(teams, list) and teams:
                            logging.info(f"🎯 ENFORCING: {stack_size_int} players from teams {teams}")
                            
                            # For each team in this stack size, check feasibility first
                            for team in teams:
                                team_batters = df[(df['Team'] == team) & (~df['Position'].str.contains('P', na=False))].index
                                if len(team_batters) >= stack_size_int:
                                    # Store constraint for later addition
                                    constraints_to_add.append((team_batters, stack_size_int, team))
                                    logging.info(f"✅ Constraint validated: At least {stack_size_int} players from {team}")
                                else:
                                    logging.error(f"❌ INFEASIBLE: Team {team} has only {len(team_batters)} batters (need {stack_size_int})")
                                    constraint_feasible = False
                
                # Only add constraints if all are feasible
                if constraint_feasible and constraints_to_add:
                    for team_batters, stack_size_int, team in constraints_to_add:
                        ...
                        team_stack_constraints_applied = True
                elif not constraint_feasible:
                    logging.error(f"❌ TEAM COMBINATION INFEASIBLE - Cannot generate lineup with current constraints")
                    return pd.DataFrame(), stack_type
            
            # Fallback to simple stacking if no team combination constraints were applied
            if not team_stack_constraints_applied and stack_type in ["5", "4", "3"]:
                stack_size = int(stack_type)
                valid_teams = []
                
                # Get available teams for stacking
                available_teams = self.team_selections.get(stack_size, [])
                if not available_teams:
                    available_teams = self.team_selections.get(str(stack_size), [])
                if not available_teams:
                    available_teams = self.team_selections.get("all", df['Team'].unique().tolist())
                ...
                if valid_teams:
                    # Randomly select 1-2 teams for diversity
                    selected_teams = random.sample(valid_teams, min(2, len(valid_teams)))
                    
                    # Add constraint: at least one of selected teams must have stack_size players
                    team_constraints = []
                    for team in selected_teams:
                        team_batters = df[(df['Team'] == team) & (~df['Position'].str.contains('P', na=False))].index
                        team_sum = pulp.lpSum([player_vars[idx] for idx in team_batters])
                        team_constraints.append(team_sum)
                    
                    # At least one team must meet the stack requirement
                    problem += pulp.lpSum([
                        pulp.LpVariable(f"team_{team}_meets_stack_{lineup_num}", cat='Binary') 
                        for team in selected_teams
                    ]) >= 1
                    
                    # For each team, if it's selected, it must have at least stack_size players
                    for i, team in enumerate(selected_teams):
                        team_var = pulp.LpVariable(f"team_{team}_meets_stack_{lineup_num}", cat='Binary')
                        team_batters = df[(df['Team'] == team) & (~df['Position'].str.contains('P', na=False))].index
                        team_sum = pulp.lpSum([player_vars[idx] for idx in team_batters])
                        
                        # If team is selected (team_var = 1), enforce stack constraint
                        problem += team_sum >= stack_size * team_var
        ...
        try:
            ...
            if problem.status == pulp.LpStatusOptimal:
                ...
                if len(selected_players) == REQUIRED_TEAM_SIZE:
                    result_lineup = df.loc[selected_players].copy()
                    total_points = result_lineup['Predicted_DK_Points'].sum()
                    total_salary = result_lineup['Salary'].sum()
                    
                    logging.debug(f"Lineup #{lineup_num+1}: {total_points:.2f} points, ${total_salary} salary")
                    return result_lineup, stack_type
                else:
                    logging.warning(f"Lineup #{lineup_num+1}: Wrong number of players selected: {len(selected_players)}")
                    return pd.DataFrame(), stack_type
            else:
                logging.warning(f"Lineup #{lineup_num+1}: Optimization failed with status: {problem.status}")
                return pd.DataFrame(), stack_type
                
        except Exception as e:
            logging.error(f"Error optimizing lineup #{lineup_num+1}: {str(e)}")
            return pd.DataFrame(), stack_type
    ...
    def setup_enhanced_risk_models(self, df_players):
        """Setup enhanced risk models using historical player data"""
        try:
            if 'fp_per_dollar' in df_players.columns:
                returns = df_players['fp_per_dollar'].fillna(df_players['fp_per_dollar'].mean())
            elif 'projected_points' in df_players.columns and 'salary' in df_players.columns:
                returns = df_players['projected_points'] / (df_players['salary'] / 1000)
                returns = returns.fillna(returns.mean())
            else:
                # Fallback to random normal returns for risk modeling
                returns = pd.Series(np.random.normal(0.05, 0.2, len(df_players)))
            
            # Setup GARCH model for volatility forecasting
            self.garch_model.fit(returns)
            
            # Calculate VaR for portfolio risk limits
            historical_returns = returns.dropna()
            if len(historical_returns) > 30:
                portfolio_var = self.var_calculator.calculate_var(historical_returns)
                portfolio_es = self.var_calculator.calculate_expected_shortfall(historical_returns)
                logging.info(f"📊 Portfolio VaR (95%): {portfolio_var:.4f}, Expected Shortfall: {portfolio_es:.4f}")
            
            # Setup factor model for risk decomposition
            factors = self._create_risk_factors(df_players)
            if factors is not None:
                self.risk_model.fit_factor_model(returns, factors)
                
            logging.info("✅ Enhanced risk models setup complete")
            
        except Exception as e:
            logging.warning(f"⚠️ Risk model setup failed: {e}, using default parameters")
    
    def _create_risk_factors(self, df_players):
        """Create risk factors from player data"""
        try:
            factors = pd.DataFrame()
            
            # Team concentration factor
            if 'team' in df_players.columns:
                team_counts = df_players['team'].value_counts()
                factors['team_concentration'] = df_players['team'].map(team_counts) / len(df_players)
            
            # Position concentration factor
            if 'position' in df_players.columns:
                pos_counts = df_players['position'].value_counts()
                factors['position_concentration'] = df_players['position'].map(pos_counts) / len(df_players)
            
            # Salary factor (high salary = lower risk)
            if 'salary' in df_players.columns:
                factors['salary_factor'] = df_players['salary'] / df_players['salary'].max()
            
            return factors if not factors.empty else None
            
        except Exception as e:
            logging.warning(f"⚠️ Risk factor creation failed: {e}")
            return None
    ...
    def calculate_lineup_risk_metrics(self, lineup):
        ...
        try:
            ...
            # Basic concentration risk
            if 'Team' in lineup.columns:
                team_counts = lineup['Team'].value_counts()
                risk_metrics['team_concentration'] = (team_counts.max() / len(lineup)) ** 2
            else:
                risk_metrics['team_concentration'] = 0.1
            
            # Salary concentration risk
            if 'Salary' in lineup.columns:
                salary_weights = lineup['Salary'] / lineup['Salary'].sum()
                risk_metrics['salary_concentration'] = (salary_weights ** 2).sum()
            else:
                risk_metrics['salary_concentration'] = 0.1
            
            # Volatility risk (using GARCH if available)
            try:
                if hasattr(self, 'garch_model') and self.garch_model.fitted:
                    # Estimate lineup volatility using GARCH forecasts
                    lineup_volatility = self.garch_model.forecast_volatility(steps=1)
                    risk_metrics['volatility'] = lineup_volatility
                else:
            ...
            if 'Predicted_DK_Points' in lineup.columns:
                expected_return = lineup['Predicted_DK_Points'].sum() / 100  # Normalize
                risk_metrics['sharpe_ratio'] = expected_return / risk_metrics['volatility']
            else:
                risk_metrics['sharpe_ratio'] = 0
            
            # VaR calculation
            try:
                lineup_returns = lineup.get('Predicted_DK_Points', pd.Series([0])) / 100
                risk_metrics['var_95'] = self.var_calculator.calculate_var(lineup_returns)
            except:
                risk_metrics['var_95'] = -0.05
            
            # Total risk score (weighted combination)
            risk_metrics['total_risk'] = (
                0.3 * risk_metrics['team_concentration'] +
                0.2 * risk_metrics['salary_concentration'] +
                0.3 * risk_metrics['volatility'] +
                0.2 * abs(risk_metrics['var_95'])
            )
            
            return risk_metrics
            
        except Exception as e:
            logging.warning(f"⚠️ Risk metrics calculation failed: {e}")
            return {'total_risk': 0.2, 'sharpe_ratio': 0, 'var_95': -0.05, 'concentration_risk': 0.1}

    def passes_enhanced_risk_checks(self, risk_metrics):
        """Enhanced risk-based filtering based on quantitative finance principles"""
        try:
            # Risk tolerance mapping
            risk_thresholds = {
                'conservative': {'max_risk': 0.15, 'min_sharpe': 0.5, 'max_var': -0.03},
                'medium': {'max_risk': 0.25, 'min_sharpe': 0.3, 'max_var': -0.05},
                'aggressive': {'max_risk': 0.40, 'min_sharpe': 0.1, 'max_var': -0.08}
            }
            
            threshold = risk_thresholds.get(self.risk_tolerance, risk_thresholds['medium'])
            
            # Apply risk filters
            if risk_metrics['total_risk'] > threshold['max_risk']:
                return False
            
            if risk_metrics['sharpe_ratio'] < threshold['min_sharpe']:
                return False
                
            if risk_metrics['var_95'] < threshold['max_var']:
                return False
            
            # Concentration limits
            if risk_metrics.get('team_concentration', 0) > 0.6:  # Max 60% in one team
                return False
                
            return True
            
        except Exception as e:
            logging.warning(f"⚠️ Risk check failed: {e}, allowing lineup")
            return True

    def optimize_with_enhanced_risk_management(self):
        """Enhanced optimization with comprehensive risk management"""
        try:
            # CRITICAL FIX: For team combination generation, we need to preserve the full count
            # Check if this is team combination generation (high lineup count + team selections)
            is_team_combo_generation = (
                self.num_lineups > 50 and  # High lineup count indicates combo generation
                self.team_selections and   # Team selections are present
                len(self.team_selections) > 0
            )
            
            if is_team_combo_generation:
                logging.info(f"🎯 TEAM COMBINATION MODE: Generating {self.num_lineups} lineups with enhanced risk metrics")
                logging.info(f"🎯 Team selections detected: {self.team_selections}")
                
                # For team combinations, we need to generate the FULL requested count
                # Call optimize_lineups but ensure it generates the full count
                original_multiplier = 2  # Save original
                
                # Temporarily modify the generation logic to get full count
                results, team_exposure, stack_exposure = self.optimize_lineups()
                
                logging.info(f"📊 Team combo generation: Got {len(results)} base lineups")
                
            else:
                # Regular optimization with smaller counts
                logging.info(f"📊 REGULAR OPTIMIZATION: Generating {self.num_lineups} lineups with risk management")
                results, team_exposure, stack_exposure = self.optimize_lineups()
            
            if not results:
                return results, team_exposure, stack_exposure
            
            logging.info(f"🔥 Enhanced risk management: Processing {len(results)} base lineups")
            
            # Apply enhanced risk filters but be more permissive to preserve lineup count
            risk_adjusted_lineups = {}
            filtered_count = 0
            
            for lineup_id, lineup_data in results.items():
                lineup_df = lineup_data['lineup']
                
                # Calculate comprehensive risk metrics
                risk_metrics = self.calculate_lineup_risk_metrics(lineup_df)
                
                # MORE PERMISSIVE: Only filter out extremely risky lineups
                # For team combinations, be even more permissive
                passes_basic_checks = True
                
                if is_team_combo_generation:
                    # For team combinations, only filter out EXTREME cases (>90% concentration)
                    if risk_metrics.get('team_concentration', 0) > 0.9:
                        passes_basic_checks = False
                        filtered_count += 1
                else:
                    # For regular optimization, use slightly stricter filter (>80% concentration)
                    if risk_metrics.get('team_concentration', 0) > 0.8:
                        passes_basic_checks = False
                        filtered_count += 1
                
                if passes_basic_checks:
                    # Add risk score to lineup
                    enhanced_lineup_data = lineup_data.copy()
                    enhanced_lineup_data['risk_score'] = risk_metrics['total_risk']
                    enhanced_lineup_data['sharpe_ratio'] = risk_metrics.get('sharpe_ratio', 0)
                    enhanced_lineup_data['var_95'] = risk_metrics.get('var_95', 0)
                    enhanced_lineup_data['concentration_risk'] = risk_metrics.get('team_concentration', 0)
                    
                    risk_adjusted_lineups[len(risk_adjusted_lineups)] = enhanced_lineup_data
                else:
                    logging.debug(f"🚫 Filtered lineup due to extreme risk: {risk_metrics}")
            
            logging.info(f"📊 Risk filtering: Kept {len(risk_adjusted_lineups)}/{len(results)} lineups (filtered: {filtered_count})")
            
            # CRITICAL: For team combinations, ensure we maintain the expected count
            if is_team_combo_generation:
                target_count = self.num_lineups
                current_count = len(risk_adjusted_lineups)
                
                if current_count < target_count * 0.9:  # If we lost more than 10%
                    logging.warning(f"🔄 Team combo mode: Risk filtering too aggressive ({current_count}/{target_count}), adding back lineups")
                    
                    # Add back the best filtered lineups to reach target
                    for lineup_id, lineup_data in results.items():
                        if len(risk_adjusted_lineups) >= target_count:
                            break
                        
                        # Check if this lineup was already included
                        lineup_key = str(lineup_data.get('total_points', 0)) + str(len(lineup_data['lineup']))
                        already_included = any(
                            str(existing.get('total_points', 0)) + str(len(existing['lineup'])) == lineup_key
                            for existing in risk_adjusted_lineups.values()
                        )
                ...
                logging.info(f"✅ TEAM COMBO COUNT PRESERVED: {len(risk_adjusted_lineups)} lineups (target: {target_count})")
            
            # Sort by risk-adjusted return (Sharpe ratio) but keep all lineups
            sorted_lineups = sorted(
                risk_adjusted_lineups.items(), 
                key=lambda x: x[1].get('sharpe_ratio', 0), 
                reverse=True
            )
            
            # Convert back to dictionary format and preserve the requested count
            final_lineups = {}
            for i, (_, lineup_data) in enumerate(sorted_lineups):
                ...
                try:
                    ...
                    lineup_data['recommended_bet_size'] = kelly_size
                    lineup_data['kelly_fraction'] = kelly_size / self.bankroll if self.bankroll > 0 else 0
                except:
                    lineup_data['recommended_bet_size'] = self.bankroll * 0.02  # 2% default
                    lineup_data['kelly_fraction'] = 0.02
                
                final_lineups[i] = lineup_data
            
            if is_team_combo_generation:
                logging.info(f"🎯 TEAM COMBINATION COMPLETE: {len(final_lineups)} lineups with enhanced risk metrics")
                logging.info(f"✅ SUCCESS: Delivered team combinations with quantitative risk analysis!")
            else:
                logging.info(f"🎯 Enhanced optimization complete: {len(final_lineups)} risk-enhanced lineups delivered")
            
            logging.info(f"✅ PRESERVED LINEUP COUNT: {len(final_lineups)} lineups with enhanced risk metrics")
            return final_lineups, team_exposure, stack_exposure
            
        except Exception as e:
            logging.error(f"❌ Enhanced optimization failed: {e}")
            return self.optimize_lineups()  # Fallback to basic optimization
...
class FantasyBaseballApp(QMainWindow):
            ...
            for teams, stack_sizes, lineups_count in selected_combinations:
                ...
                try:
                    ...
                    worker = OptimizationWorker(
                        ...
                        min_unique=0,  # FIXED: Don't use min_unique filtering with sequential optimization
                        bankroll=1000,
    ...
    def create_control_panel(self):
        ...
        self.min_unique_input.setText("0")  # Default value changed to 0 for team combinations
        self.min_unique_input.setPlaceholderText("e.g., 0")
        self.min_unique_input.setToolTip("Minimum unique players between lineups (0-10). For team combinations, use 0 since sequential optimization already ensures diversity.")
