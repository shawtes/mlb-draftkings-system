...
from sklearn.ensemble import StackingRegressor, VotingRegressor, GradientBoostingRegressor, RandomForestClassifier
    ...
    def calculate_features(self, df):
        ...
        for name, group in df.groupby('Name'):
            ...
            for col in self.stat_cols:
                ...
                # Performance vs moving average (with safe division)
                if f'{col}_sma_28' in new_features:
                    sma_28 = new_features[f'{col}_sma_28']
                    # Safe division - avoid division by zero or NaN
                    with np.errstate(divide='ignore', invalid='ignore'):
                        ratio = group[col] / sma_28
                        new_features[f'{col}_vs_sma_28'] = ratio.replace([np.inf, -np.inf], np.nan).fillna(0) - 1
            ...
            for window in self.rolling_windows:
                ...
                if mean is not None and not mean.empty:
                    # Safe division for band width
                    with np.errstate(divide='ignore', invalid='ignore'):
                        band_width = (new_features[f'dk_fpts_upper_band_{window}'] - new_features[f'dk_fpts_lower_band_{window}']) / mean
                        new_features[f'dk_fpts_band_width_{window}'] = band_width.replace([np.inf, -np.inf], np.nan).fillna(0)
                    
                    # Safe division for band position
                    with np.errstate(divide='ignore', invalid='ignore'):
                        band_range = new_features[f'dk_fpts_upper_band_{window}'] - new_features[f'dk_fpts_lower_band_{window}']
                        band_position = (group['calculated_dk_fpts'] - new_features[f'dk_fpts_lower_band_{window}']) / band_range
                        new_features[f'dk_fpts_band_position_{window}'] = band_position.replace([np.inf, -np.inf], np.nan).fillna(0.5)
            ...
            for vol_col in ['PA', 'AB']:
                if vol_col in group.columns:
                    ...
                    with np.errstate(divide='ignore', invalid='ignore'):
                        vol_ratio = group[vol_col] / new_features[f'{vol_col}_roll_mean_28']
                        new_features[f'{vol_col}_ratio'] = vol_ratio.replace([np.inf, -np.inf], np.nan).fillna(1)
                    new_features[f'dk_fpts_{vol_col}_corr_28'] = group['calculated_dk_fpts'].rolling(28).corr(group[vol_col]).fillna(0)

            # --- Interaction / Ratio Features ---
            for col in ['HR', 'RBI', 'BB', 'H', 'SO', 'R']:
                if col in group.columns and 'PA' in group.columns and group['PA'].sum() > 0:
                    # Safe division for per-PA ratios
                    with np.errstate(divide='ignore', invalid='ignore'):
                        per_pa = group[col] / group['PA']
                        new_features[f'{col}_per_pa'] = per_pa.replace([np.inf, -np.inf], np.nan).fillna(0)
...
def calculate_underdog_binary_targets(row):
    """
    Calculate binary targets for Underdog Fantasy pick predictions.
    Returns dictionary of binary outcomes for common Underdog props.
    """
    # Convert stats to numeric, defaulting to 0
    hits = pd.to_numeric(row.get('H', 0), errors='coerce')
    runs = pd.to_numeric(row.get('R', 0), errors='coerce')
    rbis = pd.to_numeric(row.get('RBI', 0), errors='coerce')
    hrs = pd.to_numeric(row.get('HR', 0), errors='coerce')
    sbs = pd.to_numeric(row.get('SB', 0), errors='coerce')
    strikeouts = pd.to_numeric(row.get('SO', 0), errors='coerce')
    
    # Common Underdog Fantasy thresholds
    targets = {
        'hits_over_0.5': int(hits >= 1),
        'hits_over_1.5': int(hits >= 2),
        'runs_over_0.5': int(runs >= 1),
        'rbis_over_0.5': int(rbis >= 1),
        'hrs_over_0.5': int(hrs >= 1),
        'sbs_over_0.5': int(sbs >= 1),
        'total_bases_over_1.5': int((hits + pd.to_numeric(row.get('2B', 0), errors='coerce') + 
                                   2 * pd.to_numeric(row.get('3B', 0), errors='coerce') + 
                                   3 * hrs) >= 2),
        'hits_runs_rbis_over_1.5': int((hits + runs + rbis) >= 2),
        'pitcher_strikeouts_over_4.5': int(strikeouts >= 5),  # For pitcher data
        'pitcher_strikeouts_over_6.5': int(strikeouts >= 7)
    }
    
    return targets

def calculate_dk_fpts(row):
    # Keep DK scoring for backwards compatibility
...
if __name__ == "__main__":
    ...
    print("Loading dataset for Underdog Fantasy training...")
    
    # Try to load from multiple possible data sources
    data_sources = [
        'C:/Users/smtes/FangraphsData/merged_fangraphs_data.csv',
        os.path.join(script_directory, '..', '7_ANALYSIS', 'battersfinal_dataset_with_features.csv'),
        'C:/Users/smtes/OneDrive/Documents/draftkings project/MLB_DRAFTKINGS_SYSTEM/7_ANALYSIS/battersfinal_dataset_with_features.csv'
    ]
    
    df = None
    for data_source in data_sources:
        try:
            print(f"Trying to load: {data_source}")
            df = pd.read_csv(data_source, low_memory=False)
            print(f"Successfully loaded data from: {data_source}")
            break
    ...
    if df is None:
        raise FileNotFoundError("No suitable data file found. Please ensure you have baseball data available.")
    
    df['date'] = pd.to_datetime(df['date'], errors='coerce')
    df.sort_values(by=['Name', 'date'], inplace=True)

    # Calculate DraftKings fantasy points for backwards compatibility
    if 'calculated_dk_fpts' not in df.columns:
        print("calculated_dk_fpts column not found. Calculating now...")
        df['calculated_dk_fpts'] = df.apply(calculate_dk_fpts, axis=1)
    
    # Calculate Underdog Fantasy binary targets
    print("Calculating Underdog Fantasy binary targets...")
    underdog_targets = df.apply(calculate_underdog_binary_targets, axis=1)
    underdog_df = pd.DataFrame(underdog_targets.tolist(), index=df.index)
    
    # Add Underdog targets to main dataframe
    for col in underdog_df.columns:
        df[col] = underdog_df[col]
    
    print(f"Added Underdog targets: {list(underdog_df.columns)}")
    ...
    # Define Underdog Fantasy target columns
    underdog_targets = [
        'hits_over_0.5', 'hits_over_1.5', 'runs_over_0.5', 'rbis_over_0.5',
        'hrs_over_0.5', 'sbs_over_0.5', 'total_bases_over_1.5', 
        'hits_runs_rbis_over_1.5', 'pitcher_strikeouts_over_4.5', 'pitcher_strikeouts_over_6.5'
    ]
    
    # Drop all target columns from features
    columns_to_drop = ['calculated_dk_fpts'] + underdog_targets
    features = df.drop(columns=[col for col in columns_to_drop if col in df.columns])
    
    # Keep DraftKings points as primary target for now, but we'll train Underdog models too
    if 'calculated_dk_fpts' in df.columns:
        primary_target = df['calculated_dk_fpts']
    else:
    ...
    features_selected = selector.fit_transform(features_preprocessed, primary_target)
    ...
    complete_pipeline.fit(features, primary_target)
    ...
    # Process features through the pipeline to get the same format as training
    processed_features = complete_pipeline.named_steps['preprocessor'].transform(features)
    selected_features = complete_pipeline.named_steps['selector'].transform(processed_features)
    ...
    mae, mse, r2, mape = evaluate_model(primary_target, all_predictions)
    ...
    model_pipeline_path = os.path.join(script_directory, 'batters_final_ensemble_model_pipeline123.pkl')
    ...
    # =============================================================================
    # TRAIN UNDERDOG FANTASY BINARY CLASSIFICATION MODELS
    # =============================================================================
    print("\n" + "="*80)
    print("TRAINING UNDERDOG FANTASY BINARY CLASSIFICATION MODELS")
    print("="*80)
    
    # Classification models and metrics are already imported at the top
    
    # Train a separate model for each Underdog prop
    underdog_models = {}
    underdog_results = {}
    
    for target_prop in underdog_targets:
        if target_prop in df.columns:
            print(f"\nTraining model for: {target_prop}")
            
            # Get the binary target
            y_binary = df[target_prop]
            
            # Check if we have at least 2 classes
            n_classes = len(y_binary.unique())
            if n_classes < 2:
                print(f"  Skipping {target_prop}: Only {n_classes} class(es) found in data (need at least 2)")
                continue
            
            # Check if we have sufficient samples of minority class
            class_counts = y_binary.value_counts()
            min_class_count = class_counts.min()
            if min_class_count < 5:
                print(f"  Skipping {target_prop}: Minority class has only {min_class_count} samples (need at least 5)")
                continue
            
            # Create classification pipeline
            classification_pipeline = Pipeline([
                ('preprocessor', preprocessor),
                ('selector', SelectKBest(f_regression, k=min(300, features_preprocessed.shape[1]))),
                ('classifier', LogisticRegression(random_state=42, max_iter=1000))
            ])
            
            # Train the classification model
            classification_pipeline.fit(features, y_binary)
            
            # Make predictions
            y_pred = classification_pipeline.predict(features)
            y_pred_proba = classification_pipeline.predict_proba(features)[:, 1]
            ...
            try:
                ...
                auc = 0.5  # Default if all predictions are the same class
            
            # Store results
            underdog_models[target_prop] = classification_pipeline
            underdog_results[target_prop] = {
                'accuracy': accuracy,
                'precision': precision,
                'recall': recall,
                'f1': f1,
            ...
            print(f"  AUC: {auc:.4f}")
            print(f"  Positive Rate: {underdog_results[target_prop]['positive_rate']:.4f}")
            
            # Save individual model
            model_path = os.path.join(script_directory, f'underdog_{target_prop}_model.pkl')
            joblib.dump(classification_pipeline, model_path)
            print(f"  Model saved to: {model_path}")
    
    # Save Underdog results summary
    underdog_summary_path = os.path.join(script_directory, 'underdog_models_summary.csv')
    underdog_summary_df = pd.DataFrame(underdog_results).T
    underdog_summary_df.to_csv(underdog_summary_path)
    print(f"\nUnderdog models summary saved to: {underdog_summary_path}")
    
    # Create Underdog predictions dataset
    print("\nCreating Underdog Fantasy predictions dataset...")
    underdog_predictions = pd.DataFrame({
        'Name': features['Name'],
        'Date': date_series
    })
    
    # Add probability predictions for each Underdog prop
    for target_prop in underdog_targets:
        if target_prop in underdog_models:
            proba_predictions = underdog_models[target_prop].predict_proba(features)[:, 1]
            underdog_predictions[f'{target_prop}_probability'] = proba_predictions
            underdog_predictions[f'{target_prop}_prediction'] = (proba_predictions > 0.5).astype(int)
    
    # Save Underdog predictions
    underdog_predictions_path = os.path.join(script_directory, 'underdog_fantasy_predictions.csv')
    underdog_predictions.to_csv(underdog_predictions_path, index=False)
    print(f"Underdog Fantasy predictions saved to: {underdog_predictions_path}")
    
    # Save comprehensive dataset with all Underdog targets and predictions
    comprehensive_underdog_path = os.path.join(script_directory, 'comprehensive_underdog_dataset.csv')
    comprehensive_df = df.copy()
    
    # Add probability predictions to comprehensive dataset
    for target_prop in underdog_targets:
        if target_prop in underdog_models:
            proba_predictions = underdog_models[target_prop].predict_proba(features)[:, 1]
            comprehensive_df[f'{target_prop}_probability'] = proba_predictions
    
    comprehensive_df.to_csv(comprehensive_underdog_path, index=False)
    print(f"Comprehensive Underdog dataset saved to: {comprehensive_underdog_path}")
    ...
    print("\n" + "="*80)
    print("UNDERDOG FANTASY TRAINING COMPLETE - SUMMARY")
    print("="*80)
    
    print(f"\n📊 Training completed in {total_time:.2f} seconds")
    print(f"\n🎯 DraftKings Model Performance:")
    print(f"   - MAE: {mae:.4f}")
    print(f"   - R²: {r2:.4f}")
    print(f"   - MAPE: {mape:.4f}%")
    
    print(f"\n🎲 Underdog Fantasy Models Trained:")
    for prop, results in underdog_results.items():
        print(f"   - {prop}:")
    ...
    print(f"\n💾 Files Generated:")
    print(f"   - DraftKings Pipeline: batters_final_ensemble_model_pipeline.pkl")
    print(f"   - Underdog Models: underdog_[prop]_model.pkl (10 models)")
    print(f"   - Predictions: underdog_fantasy_predictions.csv")
    print(f"   - Comprehensive Dataset: comprehensive_underdog_dataset.csv")
    print(f"   - Model Summary: underdog_models_summary.csv")
    
    print(f"\n🚀 Ready for Underdog Fantasy Analysis!")
    print(f"   - Use the trained models to predict player props")
    print(f"   - Load underdog_fantasy_predictions.csv for GUI integration")
    print(f"   - Models saved in: {script_directory}")
